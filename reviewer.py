"""
Phase 7: Nightly AI Reviewer — uses Gemini API to analyze 24h trading performance
...
async def run_nightly_review(session=None) -> Optional[str]:
    """Main entry point: load data, compute metrics, call Gemini, save + send."""
    date_str = datetime.now(tz=timezone.utc).strftime("%Y-%m-%d")
    log.info(f"Starting nightly review for {date_str}")

    api_key = Config.GEMINI_API_KEY
    if not api_key:
        log.warning("GEMINI_API_KEY not set — skipping nightly review")
        return None
    ...
    try:
        with open(out_path, "w") as f:
            ...
            f.write(f"_Generated by gemini-1.5-flash_\n\n")
